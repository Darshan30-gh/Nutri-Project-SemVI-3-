...
def voice_chat():
    ...
    try:
        ...
        res = client.invoke(f"Answer in {data.get('language')}: {data.get('text')}").content
        tts = gTTS(text=res, lang='en', slow=False)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as fp:
            tts.save(fp.name)
            with open(fp.name, "rb") as f: b64 = base64.b64encode(f.read()).decode('utf-8')
        return jsonify({"reply": res, "audio": b64})
    except: return jsonify({"reply": "Error", "audio": ""}), 500
